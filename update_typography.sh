...
update_typography() {
    ...
    sed -i '' 's/text-sm font-medium/text-body-sm font-axiforma/g' "$file"
    sed -i '' 's/text-sm font-semibold/text-body-sm font-axiforma/g' "$file"
    sed -i '' 's/text-xs font-medium/text-body-xs font-axiforma/g' "$file"
    
    # Update body text classes
    sed -i '' 's/text-sm text-gray-600/text-body-sm text-gray-600 font-uber/g' "$file"
    sed -i '' 's/text-xs text-gray-600/text-body-xs text-gray-600 font-uber/g' "$file"
    sed -i '' 's/text-base text-gray-600/text-body text-gray-600 font-uber/g' "$file"
    
    # Update button text
    sed -i '' 's/text-sm sm:text-base/text-button-sm sm:text-button font-uber/g' "$file"
    sed -i '' 's/text-xs transition-colors/text-button-sm font-uber transition-colors/g' "$file"
